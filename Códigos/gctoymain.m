...
if outeriter < 0.5
  [f0val,df0dx,fval,dfdx] = toy2(xval,model,index_p,len_U);
  innerit=0;
...
%%%% The results should be put in f0valnew and fvalnew.
  [f0valnew,fvalnew] = toy1(xmma,model,index_p,len_U);
...
%%%% The residual vector of the KKT conditions is calculated:
  ...
  outvector1 = [outeriter innerit xval'];
  outvector2 = [f0val fval' outeriter kktnorm]
  kktnorm
  
  %trocar pg5 por tag da visualização que tu quer
  if live_plot
      mphplot(model,'pg5')
      set(gcf,'Visible','on')
  end
...
cd('F:\GitHub\Top_op2020\Models - COMSOL')
mphsave(model,model_name);
cd('F:\GitHub\Top_op2020\GCMMA-MMA-code-1.5')
%---------------------------------------------------------------------
